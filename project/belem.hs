...
--Extend the calculator with operations for subtraction (-), integer division (/)and remainder (i.e. modulus) (%).
...
eval (ID  e1 e2) = eval e1 `div` eval e2
eval (RE  e1 e2) = eval e1 `mod` eval e2
...
eval2  env (ID  e1 e2) = eval2 env e1 `div` eval2 env e2
eval2  env (RE  e1 e2) = eval2 env e1 `mod` eval2  env e2
