...
% 1. Factos 
% São factos e não regras porque são dados estáticos, não dependem de cálculos.

% 	Para o tabuleiro ser 4x4 (ter 4 cores), basta alterar a lista 'colors' para apenas ter 4 elementos
% e ajustar os factos 'edge_1' e 'edge_2' para os novos índices.
% O resto do código é genérico e funcionaria para quaisqueres factos.
...
% Ele usa recursão para ir tirando uma cor de cada vez e construindo uma nova lista.
...
my_permutation(List, [Head | Tail]) :-
	my_select(Head, List, Rest),  % Escolhe uma cor da lista original.
	my_permutation(Rest, Tail). % Permuta o restante após remover a cor escolhida.


% -------- Seleção ------------
% Definição de Não-Detirminismo
% No 'my_select':
...
my_select(X, [X| Tail], Tail). % Seleciona o primeiro elemento da lista e remove-o do resultado.

my_select(X, [Head | Tail], [Head | Rest]) :- % Seleciona X em qualquer posição 
% posterior preservando os anteriores.
...
get_index(Color, [Color | _], 1). % Retorna 1 se a cor estiver na primeira posição da lista.
...
get_index(Color, [_ | Tail], Index) :-
	...
	get_index(Color, Tail, N),
	% Soma 1 ao índice obtido na recursão.
	Index is N + 1.

% Percorre a lista mantendo um acumulador (Current Max).
% Inicia a procura do máximo usando o primeiro elemento como acumulador.
my_max_list([H|T], Max) :- find_max(T, H, Max).

find_max([], Max, Max). % Quando chega ao fim da lista, devolve o máximo acumulado.
...
% O append serve para partir a lista.
...
%	 Sem este ! (Cut), o programa fica mais lento e pode levar a erros lógicos.
% Se X for igual a Y, o 'ignore_self' é bem sucedido. Sem o cut, o prolog tentaria
% verificar as restantes regras mesmo que o X fosse igual a Y, o que é um desperdício
% de tempo e, no caso de 'position', poderia falhar.
% 	Este é um "Green Cut" (eficiência) que se comporta como "Red Cut" neste contexto.

% Falha imediatamanete quando compara a mesma cor, evitando regras redundantes.
ignore_self(X, X) :- !.
...
% Verdadeiro se X e Y tiver um espaço entre eles.
...
position(X, _, _) :- var(X), !. % Se a variável X não tiver valor, impede que o get_index entre em loop infinito.
...
position(X, ListOfPositions, Board) :-
	...
	% Confirma se o índice caluclado está na lista de posições válidas.
...
% ----- Solve -----
% 	Primeiro gera-se uma solução candidata (my_permutation), 
% depois testa se cumpre todas as regras (satisfies_all).
% 	Se falhar, o prolog faz backtrack para o my_permutation e tenta outra.

% 	Para o número de tentativas falhadas ser devolvido, teria de se usar um contador
% dinâmico (assert / retract) cada vez que entrasse no 'satisfies_all' e falhasse.
...
% Aplica a constraint atual ao tabuleiro e continua com os restantes.
...
% ----- Best Score -----
% Para o tabuleiro correspondente ser devolvido podia se, no findall,
% Ficaria: findall(S-Board,(...), AllScores).
% Posteriormente, seria necessário adaptar o my_max_list para comparar pares
% baseando-se na chave (S).
...
% Score é o negativo de número de restrições violadas.
